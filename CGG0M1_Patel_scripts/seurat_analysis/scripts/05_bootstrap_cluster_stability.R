...
out_dir   <- file.path(project_dir, "rds", "05_bootstrap_stability")
plot_dir  <- file.path(project_dir, "plots", "05_stability")
table_dir <- file.path(project_dir, "tables")

resolutions_to_test <- c(0.3, 0.4, 0.6)
...
## matches this project's column naming (see 06_marker_evidence.R::find_cluster_col())
...
reprocess_subsample <- function(object, resolution) {
  ...
  pool_list <- lapply(pool_list, function(x) {
    ...
    SCTransform(x, vst.flavor = "v2", method = "glmGamPoi", vars.to.regress = vars_to_regress,
                variable.features.n = 3000, new.assay.name = "SCT",
                conserve.memory = TRUE, verbose = FALSE)
  })

  vf_list <- lapply(pool_list, VariableFeatures)
  ...
  pool_list <- lapply(pool_list, function(x) {
    ...
    if (length(missing_features) > 0) x <- GetResidual(x, features = missing_features, assay = "SCT", umi.assay = "RNA", verbose = FALSE)
    x
  })
...
for (resolution in resolutions_to_test) {
  ...
  message("\n== resolution ", resolution, " (", ident_col, ") ==")
  ...
  p_rain <- JaccardRainCloudPlot(idents1, idents2) +
    ...
    ggtitle(sprintf("Cluster stability (Jaccard), resolution %s, n_boot=%d, rate=%.2f",
                    res_out, length(boot_results), subsample_rate)) +
...
## ---- Cross-resolution comparison, to help pick one ----------------------
...
if (nrow(combined) == 0) {
  ...
  p_compare <- comparison_tbl %>%
    ...
    labs(x = "resolution", y = "number of clusters", color = NULL,
         title = "Total vs. stable clusters across resolutions") +
    theme_classic()
  ggsave(file.path(plot_dir, "05_resolution_comparison.png"), p_compare, width = 7, height = 5, dpi = 300)
}

message("\nDone. Outputs in ", out_dir, ", ", plot_dir, ", ", table_dir)
